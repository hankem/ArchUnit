...
ArchUnit will use exactly the `archunit.properties` file, that is returned by the context
...
E.g. to override the property `resolveMissingDependenciesFromClassPath` described in the next section, it would be possible to pass the following to the respective JVM process executing ArchUnit:
