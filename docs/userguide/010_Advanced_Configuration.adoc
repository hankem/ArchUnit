...
ArchUnit will use the `archunit.properties` file returned by the context `ClassLoader` from the classpath root
via the standard Java resource loading mechanism.

It is possible to override any property from `archunit.properties`
by passing a corresponding system property to the respective JVM process executing ArchUnit:
...
For example, the property `resolveMissingDependenciesFromClassPath` can be overridden with:
