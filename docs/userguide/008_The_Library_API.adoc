...
For example, a CI build should probably never update the violation store nor create a new one.

As mentioned in <<Overriding configuration>>, these properties can be passed as system properties as needed.
For example, using the following system property allows to create violation stores in a specific environment:
