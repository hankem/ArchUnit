...
This can help in CI environments to prevent a misconfiguration.
For example, if a CI job should never need to update the violation store, it can be prevented.
Also a misconfiguration of the store path in a CI environment can be prevented by forbidding the creation of a new violation store.

As mentioned in <<Overriding configuration>>, these these properties can be passed as system properties as needed.
